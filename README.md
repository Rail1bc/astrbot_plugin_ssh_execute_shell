...
本插件使用持久 bash session 执行命令，通过 marker 标记（`CMD_START/CMD_END`）分割输出。**单条命令的执行总时间不能超过 `timeout` 配置值（默认 30 秒）**，否则会触发超时并重建连接。

### 常见超时场景

| 操作 | 能否通过 | 说明 |
|------|---------|------|
| `ls`、`free -h` 等简单命令 | ✅ 正常 | 毫秒级完成 |
| `find / -name` 等中等命令 | ⚠️ 可能超时 | 大范围搜索建议加 `-maxdepth` 限制 |
| `wget` / `curl` 下载大文件 | ❌ 几乎必超时 | 网络速度慢时远超 30 秒阈值 |
| `apt install` 安装软件包 | ❌ 可能超时 | 含下载+解压，依赖网络速度 |
| 后台命令 `nohup wget ... &` | ✅ 可行 | 命令瞬间返回，下载进程独立于 SSH 会话 |

### 建议方案

#### 方案一：调高超时时间（推荐）

在 AstrBot WebUI → 插件管理 → SSH 命令执行器 中，将 `timeout` 配置项调大：

```yaml
timeout: 120   # 从默认 30 秒改为 120 秒
```

> ✅ 适合：已知的长时间单次操作、网络延迟较高的环境
> ⚠️ 注意：超时期间 LLM 会等待，无法响应其他消息

#### 方案二：后台下载，后期再检查

对于文件下载、软件包安装等长时间操作，采用后台进程 + 轮询检查的策略：

```bash
# 1. 后台启动下载（命令瞬间返回）
nohup wget -O /path/to/file "https://..." > /tmp/download.log 2>&1 &
...
> ✅ 适合：大文件下载、软件包安装等无法预知耗时的情况
> ✅ 优点：不受 SSH 超时限制，LLM 可在此期间处理其他消息
> ⚠️ 注意：需要轮询检查进度，无法实时获取下载日志
